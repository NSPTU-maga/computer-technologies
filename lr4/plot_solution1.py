...
def main():
    ...
    # --- Custom colormap: yellow -> white -> violet ---
    ...
    # build colormap: low = yellow, mid = white, high = violet
    cmap_custom = mcolors.LinearSegmentedColormap.from_list(
        "yellow_white_violet",
        [
            (0.0, "#ffff00"),   # yellow
            (0.5, "#ffffff"),   # white
            (1.0, "#8a2be2"),   # violet (blueviolet)
        ],
    )

    tpc = ax.tripcolor(triang, u_refined, shading="gouraud", cmap=cmap_custom, vmin=vmin, vmax=vmax)

    cb = fig.colorbar(tpc, ax=ax)
    # keep default tick colors
    cb.ax.tick_params(color="black", labelcolor="black")
