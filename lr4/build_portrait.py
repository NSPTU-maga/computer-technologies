...
def build_boundary_masks(nodes_coords,
                         ...
                         eps_r=1e-6):
    """
    Все граничные узлы (дуга + оси) считаем узлами Дирихле.
    Нейман не используется.
    """
    n = len(nodes_coords)
    mask_dirichlet = [False] * n
    mask_neumann   = [False] * n  # оставим, но использовать не будем
    ...
    for i, (x, y) in enumerate(nodes_coords):
        ...
        if on_arc or on_x_axis or on_y_axis:
...
def compute_local_rhs_quad(elem_nodes: Sequence[int],
                           nodes_coords: Sequence[Tuple[float, float]],
                           f_value: float) -> List[float]:
    """
    Локальный вектор правой части для ОБЩЕГО четырёхугольного элемента
    при константной правой части f_value.
    Используем те же 2×2 точки Гаусса и билинейное отображение,
    что и в compute_local_stiffness_quad.
    """
    assert len(elem_nodes) == 4
    ...
    for (xi, eta), w in zip(gauss_pts, gauss_w):
        # значения базисных функций в точке (xi, eta)
        N1 = 0.25 * (1 - xi) * (1 - eta)
        N2 = 0.25 * (1 + xi) * (1 - eta)
        N3 = 0.25 * (1 + xi) * (1 + eta)
        N4 = 0.25 * (1 - xi) * (1 + eta)
        ...
        # производные для детерминанта Якоби
        dN1_dxi  = -0.25 * (1 - eta)
        dN1_deta = -0.25 * (1 - xi)
        dN2_dxi  =  0.25 * (1 - eta)
        dN2_deta = -0.25 * (1 + xi)
        ...
        for iN in range(4):
            b_std[iN] += f_value * N[iN] * detJ * w

    # возвращаем к исходному порядку elem_nodes
    pos_in_std = {nid: i for i, nid in enumerate(nid_std)}
    b_elem = [0.0] * 4
    for i_e, nid_i in enumerate(elem_nodes):
        i_s = pos_in_std[nid_i]
        b_elem[i_e] = b_std[i_s]
...
def main():
    ...
    f_value = -4.0   # правая часть уравнения -Δu = -4
    ...
    for elem_nodes, _mat in elements:
        ...
        b_loc = compute_local_rhs_quad(elem_nodes, nodes, f_value)
        for i_loc, node_i in enumerate(elem_nodes):
            b[node_i] += b_loc[i_loc]
    ...
    for i, (x, y) in enumerate(nodes):
        ...
        if err > max_err:
            max_err = err
    print(f"Максимальная ошибка по тесту u = x^2 + y^2: {max_err:.3e}")

    with open("solution_lr3_test_r2.txt", "w", encoding="utf-8") as fsol:
