...
class MeshData:
    def __init__(self):
        ...
        self.Kx = 0
        self.Ky = 0
        self.coord_lines = []  # Ky строк по Kx точек (x,y), упорядочены слева->направо, снизу->вверх
        self.subregions = []
        self.nodes = []        # список (x,y)
        self.elements = []     # список [n1,n2,n3,n4,mat]
        self.n_radial_div = 5  # число слоёв от квадрата до дуги (включая квадрат и дугу)
        self.spacing = 1.0     # геометрическое распределение по слоям (1.0 — равномерно)
...
def read_area_file(filename):
    ...
    for i in range(mesh.Ky):
        ...
        mesh.coord_lines.append(line_points)   # предполагаем: уже слева->направо
        cur += 1
...
# ------------- генератор сетки (переписан) -------------
def generate_radial_mesh(mesh):
    s = mesh.square_size
    R = mesh.radius
    Kx, Ky = mesh.Kx, mesh.Ky
    L = mesh.n_radial_div            # число “радиальных уровней” (включая 0: квадрат, L-1: дуга)
    assert L >= 2, "n_radial_div должно быть >=2"

    # --- 0) базовые ряды квадрата ---
    # nodes: построчно снизу->вверх, внутри ряда слева->направо
    nodes = []
    for i in range(Ky):
        row = mesh.coord_lines[i]
        row_sorted = sorted(row, key=lambda p: p[0])  # на всяк. случай
        nodes.extend(row_sorted)
    base_count = len(nodes)  # Kx*Ky

    # --- 1) дуговые точки-цели: правая полоса (Ky штук) + верхняя полоса (Kx штук) ---
    right_sq = [mesh.coord_lines[i][-1] for i in range(Ky)]          # нижняя->верхняя
    top_sq   = mesh.coord_lines[-1]                                  # слева->направо

    arc_right = []  # соответствует right_sq (по i: 0..Ky-1)
    for i in range(Ky):
        y = right_sq[i][1]
        ratio = y / s                        # 0..1
        ang = (math.pi/4) * ratio            # 0 .. pi/4
        arc_right.append([R*math.cos(ang), R*math.sin(ang)])

    arc_top = []   # соответствует top_sq (по j: 0..Kx-1)
    for j in range(Kx):
        x = top_sq[j][0]
        ratio = 1 - x/s                      # 1..0 по мере движения слева->направо
        ang = (math.pi/4) + (math.pi/4)*ratio   # pi/2..pi/4 при j:0..Kx-1 (слева->направо)
        arc_top.append([R*math.cos(ang), R*math.sin(ang)])

    # --- 2) интерполяционные уровни между квадратом и дугой ---
    # слои 1..L-2 (если есть), для правой и верхней полос отдельно
    def interp_t(layer_idx):
        # равномерно или с геометрическим шагом (spacing)
        if mesh.spacing == 1.0:
    ...
    right_layers = []   # список слоёв; каждый слой — список Ky точек (снизу->верх)
    top_layers   = []   # список слоёв; каждый слой — список Kx точек (слева->право)
    ...
    for layer_idx in range(1, L-1):
        ...
        right_layers.append(rlayer)  # порядок: снизу->верх
        ...
        top_layers.append(tlayer)    # порядок: слева->право

    # --- 3) глобальная НУМЕРАЦИЯ УЗЛОВ строго по слоям (как в методичке) ---
    # квадрат (уже добавлен) → все промежуточные слои правой полосы (снизу->верх),
    # затем промежуточные слои верхней полосы (слева->право),
    # затем внешняя правая дуга (снизу->верх), затем внешняя верхняя дуга (слева->право)
    # (так мы сохраняем «строковую» логику для каждой полосы отдельно)

    # индекстаблицы для быстрой адресации при построении элементов
    idx_right_layer = []  # для слоёв 1..L-2: список индексов узлов длиной Ky
    idx_top_layer   = []  # для слоёв 1..L-2: список индексов узлов длиной Kx

    # 3.1 промежуточные слои правой полосы
    for rlayer in right_layers:
        start = len(nodes)
        nodes.extend(rlayer)
        idx_right_layer.append(list(range(start, start+Ky)))

    # 3.2 промежуточные слои верхней полосы
    for tlayer in top_layers:
        start = len(nodes)
        nodes.extend(tlayer)
        idx_top_layer.append(list(range(start, start+Kx)))

    # 3.3 внешняя дуга (правая и верхняя)
    start_arc_right = len(nodes)
    nodes.extend(arc_right)                    # Ky штук (снизу->верх)
    idx_arc_right = list(range(start_arc_right, start_arc_right+Ky))

    start_arc_top = len(nodes)
    nodes.extend(arc_top)                      # Kx штук (слева->право)
    ...
    # 4.2 правая полоса: столбец квадратной правой кромки ↔ слои ↔ дуга
    col_sq_right = [i*Kx + (Kx-1) for i in range(Ky)]  # снизу->верх

    # слои между квадратом и дугой по правой кромке
    def right_col(layer_step):  # layer_step: 0..(L-1), где 0 — квадрат, L-1 — дуга
        if layer_step == 0:
            return col_sq_right
        if layer_step == (L-1):
    ...
    # 4.3 верхняя полоса: верхний ряд квадрата ↔ слои ↔ дуга
    row_sq_top = [(Ky-1)*Kx + j for j in range(Kx)]  # слева->право
    ...
    # 4.4 — убрать возможные дубликаты в узлах (совпадение углов и т.п.)
